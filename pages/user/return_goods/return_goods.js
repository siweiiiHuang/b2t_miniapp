...
Page({
    data: {
        ...
        checkBtns: ["仅退款", "退货退款", "换货"],//服务按钮tab
        typeBtnIndex:0,  //服务按钮索引
        applyNum:1,//申请数量
        isReceive: false,
        causeCont:'注意保持商品的完好,建议您先与卖家沟通',  //提交原因，默认提示文字
        causeList:["订单不能按预计时间送达", "操作有误（商品、地址等选错）", "重复下单/误下单", "其他渠道价格更低", "该商品降价了", "不想买了"],
        checkCauseIndex:-1,
        popState:true,
        description: '',
    },
    ...
    checkSubmit: function(call) {
        if (this.data.checkCauseIndex < 0) {
            return app.confirmBox("请先选择提交原因");
        }
        if (this.data.description.length == 0) {
            return app.confirmBox("请先填写问题描述");
        }
        if (this.data.filePaths.length == 0) {
            return wx.showModal({
                title: '确定不上传图片吗？',
