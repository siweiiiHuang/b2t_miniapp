...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.removeStorageSync('order:shop_order_list:update');
  },

  onShow: function () {
    if (wx.getStorageSync('order:shop_order_list:update')) {
      wx.setStorageSync('order:shop_order_list:update', false);
      this.resetData();
  ...
  requestOrderList: function (categoryId) {
    var that = this;
    var requestUrl = that.data.url + '/api/order/shopOrderList';
    var tabType = '';
    if (categoryId == 1) {
      tabType = 'WAITPAY';
