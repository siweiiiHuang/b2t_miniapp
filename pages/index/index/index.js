...
Page({
    ...
    onLoad: function(options) {
        ...
        app.getConfig(function (res) {    
            ...
            var wap_home_logo = common.getFullUrl(common.getConfigByName(res.config, 'wap_home_logo'));   //首页logo
    ...
    requestRecommend: function () {
        ...
        load.request(requestUrl, function (res) {
          for (var ar of res.data.result){
            var index = 0;
            ar.shop_price = ar.shop_price.split('.');
            res.data.result[index] = ar;
            index++;
          }
        ...
        request.get('/api/index/homePage?new_ad=1', {
            ...
            success: function (res) {
                ...
                if (sale_goods.length > 0 && sale_goods.length < 3){
                  var j = 0;
                  for (var i of sale_goods){
                    sale_goods[j].price = i.price.toFixed(2).toString().split('.')
                    j++
                  }
    ...
    adTopage: function (e) {
        var media_type = e.currentTarget.dataset.type;
        var ad_link = e.currentTarget.dataset.link;
        if (media_type == 3) {
            if (ad_link != '') {
                wx.navigateTo({ url: '/pages/goods/goodsInfo/goodsInfo?goods_id=' + ad_link });
            }
        } else if (media_type == 4) {
            if (ad_link != '') {
                wx.navigateTo({ url: '/pages/goods/goodsList/goodsList?cat_id=' + ad_link });
            }
        } else if (media_type == 5) {
            if (ad_link != '') {
                this.setData({ webUrl: ad_link });
